...
public class MapDistanceFixCommand {
    ...
    public static void register(
        ...
        LiteralArgumentBuilder<CommandSourceStack> root = Commands.literal(
            "mapdistancefix"
        ).requires(MapDistanceFixCommand::checkPermission);
    ...
    private static boolean checkPermission(CommandSourceStack source) {
        // Try 1.21 and below: CommandSourceStack.hasPermission(int)
        for (String methodName : new String[] {
            ...
            try {
                java.lang.reflect.Method hasPerm =
                    CommandSourceStack.class.getMethod(methodName, int.class);
                return (Boolean) hasPerm.invoke(source, 2);
            } catch (Exception ignored) {}
        }

        // 1.21.1+ fallback: Commands.hasPermission(Commands.LEVEL_GAMEMASTERS)
        for (String fieldName : new String[] {
            ...
            try {
                java.lang.reflect.Field field = Commands.class.getField(
                    fieldName
                );
                Object permissionCheck = field.get(null);
                for (java.lang.reflect.Method m : Commands.class.getMethods()) {
                    if (
                        m.getParameterCount() == 1 &&
                        m.getParameterTypes()[0] == field.getType() &&
                        java.util.function
                            .Predicate.class.isAssignableFrom(m.getReturnType())
                    ) {
